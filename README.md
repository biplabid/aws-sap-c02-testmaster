...
-   **Mock Exam**: A full 75-question, 3-hour simulation of the SAP-C02 exam, with a question palette for navigation, the ability to mark questions for review, a pre-submission review screen, and a results summary showing your score and a per-question correct/wrong/unanswered/marked breakdown. As in Timed Quiz, every question in the results list shows all of its answer options with your selection and the correct answers marked, and the Correct/Incorrect tiles filter the results and add an **Explanation** toggle to each question you got wrong. Before starting, you choose where the paper comes from: **all question banks** (every bank in `data/` combined into one pool) or **a single question bank** (only banks holding 75+ questions are offered, since a full paper needs that many). Either way the 75 questions are drawn fresh each attempt, weighted to the official SAP-C02 domain split — 26% / 29% / 25% / 20%, i.e. 19 / 22 / 19 / 15 questions — with any shortfall in a thin domain topped up from the rest of the pool, and the domain mix actually drawn is reported before you start.
